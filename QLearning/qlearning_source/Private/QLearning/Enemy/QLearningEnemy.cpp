...
void AQLearningEnemy::UpdateQState_Target(float TargetHealthPercent, bool bIsTargetAttacking, bool bIsTargetGuarding,
	bool bIsTargetDodging)
{
...
/*void AQLearningEnemy::UpdateQState_Target(const float TargetHealthPercent, const int8 bIsTargetAttacking, const int8 bIsTargetGuarding, const int8 bIsTargetDodging)
{
	if (const float HealthPercent = Attributes->GetHealthPercent())
	{
		QState->HealthPercent = static_cast<uint8>(100 * HealthPercent);
	}
	if (TargetHealthPercent >= 0.f)
	{
		QState->TargetHealthPercent = static_cast<uint8>(100 * TargetHealthPercent);
	}
	if (bIsTargetAttacking != -1)
	{
		QState->bIsTargetAttacking = static_cast<bool>(bIsTargetAttacking);
	}
	if (bIsTargetGuarding != -1)
	{
		QState->bIsTargetGuarding = static_cast<bool>(bIsTargetGuarding);
	}
...
void AQLearningEnemy::UpdateQState_EnemyHealthPercent(const float TargetHealthPercent)
{
	QState->TargetHealthPercent = static_cast<uint8>(100 * TargetHealthPercent);
}

void AQLearningEnemy::UpdateQState_Distance()
{
	if (!QTarget) return;
...
void AQLearningEnemy::UpdateQState_IsEnemyAttacking(const bool bIsTargetAttacking)
{
	QState->bIsTargetAttacking = bIsTargetAttacking;
}

void AQLearningEnemy::UpdateQState_IsEnemyGuarding(const bool bIsTargetGuarding)
{
	QState->bIsTargetAttacking = bIsTargetGuarding;
}
...
void AQLearningEnemy::Die()
{
	...
	UpdateFunction_Phase2(); // test placement (apply death penalty to last action immediately)
	//ClearFallbackPhase2Timer();
}
